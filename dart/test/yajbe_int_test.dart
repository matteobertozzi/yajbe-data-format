...
void main() {
  group('YAJBE Int Tests', () {
    ...
    test("Test Rand Int Encode/Decode", () {
      Random rand = Random();
      for (int i = 0; i < 1000; ++i) {
        int input = rand.nextInt(2147483647);
        Uint8List enc = yajbeEncode(input);
        expect(input, yajbeDecodeUint8Array(enc));
      }
    });

    test("Test Rand Long Encode/Decode", () {
      Random rand = Random();
      for (int i = 0; i < 1000; ++i) {
        int input = (rand.nextDouble() * 9223372036854775807).floor();
        Uint8List enc = yajbeEncode(input);
        expect(input, yajbeDecodeUint8Array(enc));
      }
    });

    test("Test Rand IntArray Encode/Decode", () {
      Random rand = Random();
      for (int k = 0; k < 32; ++k) {
        int length = rand.nextInt(1 << 15);
        var input = [];
        for (int i = 0; i < length; ++i) {
          input.add(rand.nextInt(2147483647));
        }
        Uint8List enc = yajbeEncode(input);
        expect(input, yajbeDecodeUint8Array(enc));
      }
    ...
    test("Test Rand LongArray Encode/Decode", () {
      Random rand = Random();
      for (int k = 0; k < 32; ++k) {
        int length = rand.nextInt(1 << 15);
        var input = [];
        for (int i = 0; i < length; ++i) {
          input.add((rand.nextDouble() * 9223372036854775807).floor());
        }
        Uint8List enc = yajbeEncode(input);
        expect(input, yajbeDecodeUint8Array(enc));
      }
